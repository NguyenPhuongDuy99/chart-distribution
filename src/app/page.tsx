...
export default function Home() {
  ...
  const getTransferLogs = async (addresses: string[]): Promise<TransferLogData> => {
    ...
    logs.forEach((log) => {
      if (log.args.to && log.args.value && addresses.includes(log.args.to.toLowerCase())) {
        addressesInOutLogs[log.args.to.toLowerCase()].in = addressesInOutLogs[log.args.to.toLowerCase()].in + BigInt(log.args.value);
        addressesInOutLogs[log.args.to.toLowerCase()].remaining = addressesInOutLogs[log.args.to.toLowerCase()].remaining + BigInt(log.args.value);
      }
      if (log.args.from && log.args.value && addresses.includes(log.args.from.toLowerCase())) {
          addressesInOutLogs[log.args.from.toLowerCase()].out = addressesInOutLogs[log.args.from.toLowerCase()].out + BigInt(log.args.value);
